...
ll ceil_division(ll num, ll den){
    return (num + den - (ll)1) / den;
}

__device__ ll fastMult( ll base, ll factor, ll m ){
    ll ret = 0, current = base;
    while(factor > 0){
        int r = factor%((ll) 2);
...
ll hostFastMult( ll base, ll factor, ll m ){
    ll ret = 0, current = base;
    while(factor > 0){
        int r = factor%((ll) 2);
        if( r == 1 ) ret = (ret+current)%m;
...
ll hostFastExpo( ll base, ll expo, ll m ){
    if( expo == 0 ) return 1;
    ll prv_expo = expo/(ll)2;
    ll ret = hostFastExpo(base, prv_expo, m);
    ret = hostFastMult(ret,ret,m);
    if( expo%(ll)2 ) ret = hostFastMult(ret, base, m);
    return ret;
}

__device__ ll fastExpo( ll base, ll expo, ll m ){
    ll ret = 1, current = base;
    while(expo > 0){
        int r = expo%((ll) 2);
        if( r == 1 ) ret = fastMult(ret,current,m);
        expo /= (ll) 2;
        current = fastMult(current, current, m);
    }
    return ret;
}

__device__ ll function_1( ll a, ll n, ll p, ll m ){
    ll expo = fastMult(n , p , m);
    return fastExpo(a, expo, m);
}

__device__ ll function_2( ll a, ll b, ll q, ll m ){
    ll ret = fastExpo(a, q, m);
    ret = fastMult(ret , b , m);
    return ret;
}
...
__global__ void calculateFunction1(long long *a, long long *m, long long *n, long long *limit, long long *step, 
    ...
    ll low = (*step) * threadId + (ll)1;
    ll high = low + (*step) - (ll)1;
    ...
    for(ll p = low ; p <= high ; p ++) {
        ll value = function_1((*a), (*n), p, (*m));
        // if((p%(ll)10000) == 0) printf("\t --> %d TRY p = %lld\n",threadId, (long long)p);
        // INSERT TO RESULTS
...
__global__ void calculateFunction2(long long *a, long long *b, long long *m, long long *n,
    ...
    ll low = (*step) * threadId;
    ll high = low + (*step) - (ll)1;
    if( threadId + 1 == numThreads ) high = (*limit);
    for(ll q = low ; q <= high ; q ++) {
        ...
        if( findP == -1 ) continue;
        ll currentX = fastMult((*n) , findP , (*m));
